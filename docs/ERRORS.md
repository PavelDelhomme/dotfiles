...
| **Philips 245E1 (DP-1)** : après extinction bouton power puis rallumage, layout bascule sur le petit **HP (HDMI-A-2)**, perte du **2560×1440** ; seul un débranchement DisplayPort physique répare | (1) Power-off moniteur = **hot-unplug DP** ; KWin réassigne primary / fenêtres vers les sorties restantes (souvent le HP). (2) Au reconnect, **NVIDIA propriétaire + DP** renégocie parfois un EDID/mode incorrect jusqu'à un vrai unplug/replug. | `displayman doctor` puis `displayman fix` (soft-replug `DP-1` disable→enable + layout cible Xiaomi\|Philips\|HP via `kscreen-doctor`). Préférer veille OS / DPMS plutôt que bouton power. Les fenêtres déjà sur le HP ne reviennent pas toutes seules. Voir [`docs/guides/SCREEN_DISPLAY.md`](guides/SCREEN_DISPLAY.md) § hotplug DP. | outillé *(2026-09-20 — `displayman layout/doctor/fix`)* |
