#!/bin/bash
# =============================================================================
# MANMAN - Manager of Managers pour Bash
# =============================================================================
...
manman() {
    ...
    local DOTFILES_FUNCTIONS_DIR="${DOTFILES_DIR:-$HOME/dotfiles}/zsh/functions"
    ...
    for manager_info in "${managers[@]}"; do
        IFS='|' read -r info command <<< "$manager_info"
    ...
    if [ "$choice" -ge 1 ] && [ "$choice" -le ${#managers[@]} ]; then
        ...
        local selected_manager="${managers[$array_index]}"
        IFS='|' read -r info command <<< "$selected_manager"
        IFS=':' read -r name description <<< "$info"
        ...
        echo
        read -n 1 -p "Appuyez sur une touche pour retourner au menu... "
        echo
        manman
    else
