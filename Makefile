...
test-updateman-cursor-release: ## Smoke API Cursor + updateman cursor check (non destructif)
	@DOTFILES_DIR="$(DOTFILES_DIR)" bash "$(SCRIPT_DIR)/test/updateman_cursor_release_smoke.sh"

test-updateman-registry-smoke: ## Smoke updateman registre (cursor/docker/brave) en Docker isole
	@DOTFILES_DIR="$(DOTFILES_DIR)" bash "$(SCRIPT_DIR)/test/docker/test_updateman_registry_smoke.sh" "$${DISTRO:-debian}"
