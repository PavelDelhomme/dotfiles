...
################################################################################
# Installation complète Zsh + Oh My Zsh + Powerlevel10k + Nerd Fonts
# Script d'installation tout-en-un à la racine des dotfiles
# 
# Ce script installe et configure automatiquement :
# - Zsh
# - Oh My Zsh
# - Powerlevel10k (thème avec support Git)
# - Plugins Zsh (autosuggestions, syntax-highlighting, completions)
# - Nerd Fonts (MesloLGS NF pour support emojis/icônes)
# - Configuration automatique du terminal
# 
# Usage: bash ~/dotfiles/install_zsh_complete.sh
################################################################################
...
# Déterminer le répertoire des dotfiles
...
log_section "Installation complète Zsh + Oh My Zsh + Powerlevel10k + Nerd Fonts"
...
if [ -f "$SCRIPT_DIR/config/install_zsh_plugins.sh" ]; then
    bash "$SCRIPT_DIR/config/install_zsh_plugins.sh" || {
...
if [ -f "$SCRIPT_DIR/config/setup_p10k.sh" ]; then
    bash "$SCRIPT_DIR/config/setup_p10k.sh" || {
...
################################################################################
# ÉTAPE 7: Installation de Nerd Fonts
################################################################################
log_section "7. Installation de Nerd Fonts"

if [ -f "$SCRIPT_DIR/config/install_nerd_fonts.sh" ]; then
    bash "$SCRIPT_DIR/config/install_nerd_fonts.sh" || {
        log_warn "⚠️  Installation des Nerd Fonts échouée"
    }
else
    log_warn "⚠️  Script install_nerd_fonts.sh non trouvé"
fi

################################################################################
# ÉTAPE 8: Configuration automatique du terminal
################################################################################
log_section "8. Configuration automatique du terminal"

if [ -f "$SCRIPT_DIR/config/configure_terminal_font.sh" ]; then
    bash "$SCRIPT_DIR/config/configure_terminal_font.sh" || {
        log_warn "⚠️  Configuration du terminal échouée"
    }
else
    log_warn "⚠️  Script configure_terminal_font.sh non trouvé"
fi

################################################################################
# ÉTAPE 9: Vérification du symlink .zshrc
################################################################################
log_section "9. Vérification du symlink .zshrc"
...
################################################################################
# ÉTAPE 10: Définir Zsh comme shell par défaut
################################################################################
log_section "10. Configuration du shell par défaut"
...
echo "  ✅ Plugins Zsh installés (autosuggestions, syntax-highlighting, completions)"
echo "  ✅ Nerd Fonts installées (MesloLGS NF)"
echo "  ✅ Configuration Git dans le prompt"
echo "  ✅ Symlink .zshrc configuré"
echo "  ✅ Terminal configuré pour afficher les emojis/icônes"
echo ""
echo "📝 Prochaines étapes:"
echo "  1. Redémarrez complètement votre terminal (fermez toutes les fenêtres)"
echo "  2. Les emojis et icônes devraient maintenant s'afficher correctement"
echo "  3. Pour reconfigurer le prompt: p10k configure"
echo ""
echo "💡 Si les icônes ne s'affichent pas:"
echo "  - Vérifiez que la police MesloLGS NF est sélectionnée dans votre terminal"
echo "  - Redémarrez complètement le terminal"
echo ""
