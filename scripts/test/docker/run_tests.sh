...
if [ -f "$DOTFILES_DIR/scripts/test/lib/dotfiles_test_config.sh" ]; then
    ...
    MIGRATED_MANAGERS="pathman manman searchman aliaman installman updateman configman gitman fileman helpman cyberman devman virtman miscman doctorman netman sshman testman testzshman moduleman multimediaman cyberlearn displayman diffman diskman"
    UNMIGRATED_MANAGERS=""
fi
