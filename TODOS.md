...
- [~] **P5 / P8c — installman + registre updateman** (lot actuel après validations TODOS) :
  ...
  - **P8c** : **docker + brave** dans `updatable-tools.list` ; `updateman <outil> status|check|run` (pkg) ; smoke Docker isole `make test-updateman-registry-smoke` ; **`updateman cursor status`** enrichi (versions + timer). **Reste** : timers systemd docker/brave ; autres outils.
