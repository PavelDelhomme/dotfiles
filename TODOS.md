...
- [~] **P5 / P8c — installman + registre updateman** (lot actuel après validations TODOS) :
  - **P5** : modules installman multi-distro (`docs/managers/INSTALLMAN_VISION.md`) — matrice backends, `installman doctor`.
  - **P8c** : **docker + brave** dans le registre + timers `docker-update` / `brave-update` (hebdo, **auto_service=0** — activation hote apres validation utilisateur). Smoke Docker isole : `make test-updateman-registry-smoke`. **Reste** : autres outils registre ; bascule `auto_service=1` sur hote apres OK.
...
- [~] Exécuter et remplir **[`docs/TESTS.md`](docs/TESTS.md)** (procédure ordonnée + cases à cocher) ; menu d’appui : **`make tests-start`** ; copie commandes : **`make tests-copy STEP=…`**.
  - **Avancement 2026-06-16** : **G.0→G.0.f**, tableau **G.1–G.27**, **H.1–H.3**, **I.1–I.2** validés. Merge **`feat/manual-tests-g0` → `dev`**. Branche QA **`test/manual-tests-g0`**. **P11** : workflow `ci-updateman-smoke.yml`. **Validations TODOS** : tableau « Finalisées — en attente » **clos 2026-06-15** (délégation utilisateur). **Suite active** : **P5** / **P8c** → **preprod → main** → **P15 shellman**.
