...
- [~] **P3b-b — interfaces terminal adaptatives** : bannieres `manager_ui_print_banner` generalisees (devman, virtman, sshman, displayman, helpman, processman, routeman, configman, gitman, moduleman, multimediaman, cyberman, aliaman, searchman, miscman, testzshman, cyberlearn + pilotes precedents). **Reste** : `manager_ui_section_rule`, `tui_truncate` / `tui_menu_height`, pathman/doctorman, tests `stty cols 60` / EXT-002.
