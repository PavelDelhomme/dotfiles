...
- [~] **P3b-a — restructuration UI / menus avant adaptatif** (prioritaire) : `shared/` vs `share/`, `scripts/menu/README.md` + LEGACY, `make bootstrap-menu` → `setup.sh`, selection via `manager_ui_select_file` (**netman**, **aliaman**, **cyberlearn**, **searchman**, **miscman**, **testzshman**, **cyberman**). **Reste** : bannieres autres `*man`, menus inline sans wrapper, decision declaratif `share/menus` vs core.
