...
- [~] **P3b-a — restructuration UI / menus avant adaptatif** (prioritaire) : clarifier `shared/` (code/config) vs `share/` (donnees declaratives), isoler `scripts/menu/*.sh` en legacy/bootstrap, garder `shells/*/adapters` minces, centraliser la selection dans `scripts/lib/manager_ui.sh` (`manager_ui_select_file`, `manager_ui_select`). Doc : [`docs/architecture/UI_MENU_RESTRUCTURE.md`](docs/architecture/UI_MENU_RESTRUCTURE.md) + [`core/managers/MANAGERS_UI.md`](core/managers/MANAGERS_UI.md). Pilotes selection : **netman**, **aliaman**, **cyberlearn**.
- [ ] **P3b-b — interfaces terminal adaptatives** : apres P3b-a, generaliser `manager_ui_print_banner`, `manager_ui_section_rule`, `tui_truncate`, `tui_menu_height` aux autres `*man` (~15), puis tests `stty cols 60` / EXT-002.
