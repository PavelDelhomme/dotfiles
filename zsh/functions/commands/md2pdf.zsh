#!/bin/zsh
# =============================================================================
# MD2PDF - Conversion Markdown vers PDF avec style
# =============================================================================
...
# DESC: Convertit un fichier Markdown en PDF avec style
# USAGE: md2pdf <fichier.md> [options]
# USAGE: convert <fichier.md> [options]
# EXAMPLE: md2pdf README.md
# EXAMPLE: convert document.md --output mon_document.pdf
# EXAMPLE: md2pdf README.md --style github
md2pdf() {
    local RED='\033[0;31m'
    ...
    if [ "$show_help" = true ] || [ -z "$input_file" ]; then
        ...
        echo "  -o, --open                 Ouvrir le PDF après conversion"
    ...
    if [[ ! "$input_file" =~ \.(md|markdown)$ ]]; then
        ...
        read -q "REPLY?Continuer quand même? (y/N): "
        echo ""
        if [[ ! "$REPLY" =~ ^[yY]$ ]]; then
            return 1
        fi
...
if ! command -v convert &>/dev/null || [[ "$(which convert)" == *"md2pdf"* ]]; then
